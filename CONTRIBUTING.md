> **Canonical source:** https://git.communitycoins.org/Communitycoins.Rooty/README/src/branch/main/CONTRIBUTING.md  
> Mirrors (GitLab/GitHub) may be read-only and may lag behind.

# CONTRIBUTING

This Communitycoins hub/library operates on [Forgejo](https://forgejo.org/), an open source Gitea fork. The library serves to maintain and collect multiple projects that are releated to Communitycoins. 

The main structure in Forgejo is an Organization. Organizations are not technically interconnected, but each contains a set of interconnected repositories. A repository is a hierarchical set of files, usually to bundle a software project. One special file (README.md) describes the purpose of the repository contents.

Normaly there is no organization of Organizations and only maintainers can add organisations. For clarity we maintain a single Organization named [Communitycoins.Rooty](/Communitycoins) that contains active repositories. One repository named [README](https://git.communitycoins.org/Communitycoins/README) serves to bundle directions for readers and those that want to contribute. 

Since a software project is never finished Forgejo serves to maintain project-issues, -branches, -tags and -releases. Before anything changes you need to tell us why. Your answer becomes an issue. Before you start to work on an issue or a set of issues you need to create a branch, which is a named copy of all the files in a repository. Once the issues that caused a branch are finished the changes can be 'pulled' back in the original through a pull request. That is the way a repository changes. After a change it is possible to define a tag. Through tags it is possible to rewind the project to a previous state or to declare a release.

Forgejo also uses the name project which is confusing because a Forgejo-project is not a repository but a set of issues. So you cannot define a forgeo-project when there are no issues. A Forgejo project is a management board to collect and manage issues within an organization.

## internal links, a compromize

Regretfully git-hubs are not perfect with regard to internal references. This counts for Forgeo, as well as Gitlab and Github. A reference to a file looks like this:
```
<url>/<organization>/<repo>/<git-internal>/<filename>
url==https://git.communitycoins.org
organization==Communitycoins.Rooty
repo==README
git-internal==/src/branch/main
filename=Documentation%20and%20Governance/Editorial%20Process%20for%20Rooty%20Documents.md
```
Because this requires hard-coding, the mirroring of documentation is not perfect. Therefor, these documents need to clearly reference their canonical source such as 
> **Canonical source:** https://git.communitycoins.org/Communitycoins.Rooty/README/src/branch/main/CONTRIBUTING.md  
> Mirrors (GitLab/GitHub) may be read-only and may lag behind.
When creating a fork, all references need to be reworked.

## Mirrors

Since decentralization is the goal of the communitycoin initiative

## Policy

Artifacts and container images are ephemeral by design.
The source code, tags, and build instructions are the sole canonical record.
All binaries are reproducible and may expire without notice.

## Continuity

All repositories are standard Git repositories.
Any team may mirror any repository at any time.

In the event that git.communitycoins.org becomes unavailable,
the project continues from existing mirrors.


